...
(
    ([一-龯]{1,2}|[゠-ヿ]{2,12}|こと|ところ|[一-龯](?:[ぁ-ゖ゛-ゟ](?!で))+[一-龯]|[゠-ヿ]{2,12}[一-龯]|もの|入り|」|たち|ここ|そこ|[一-龯]ら|(?P<double>[ぁ-ゖ゛-ゟ]{2})(?P=double)|[えけげせぜてでねめれ]る|まま|[あこそ]いつ|あ[なん]た)
    (が(?!(して|った))|か(?!([はもらえけげせぜてでねめれいきぎしちにんをうくぐすつぬむる]|った))|か[は]|は(?!ず)|も|の(?![みにがはた為])|なく(?!て)|な(?![くのんらい])|する(?!な)|から(?!して)|まで|に(?!([はも]|ついて|関して|すら))|
    に[はも]|へ|で(?![はもすきの])|で[はも]|じて|や|と[のは]|と(?!([のなは]|[い言云]う))|して[はも]|して(?![はも])|ならば|なら(?![ばで]))
    |
    [、。？！・：；]
    |
    (――)|(……)|(\.\.\.)
    ...
    すなわち|[うくぐすつぬふむる]の[にはもが]|を|んな[のに]|ったら|として|つまり|ちょっと|ちょうど|々な|々に(?![もは])|々に[もは]|たい(?=[一-龯])|けど|
    だと(?!は)|だとは|とは|[のただ]ほうが|ないほうが|[のただ]方が|ない方が|風に|[いきしちにひみり]たくて|[うくすつぬふむる]まて|[^一-龯]続く|ないと(?=いけ)|
    ほとんど|らしくて(?!は)|らしく(?!て)|ため([にの](?![はも])|ならば|なら(?!ば))|ため[にの][はも]|為に(?![はも])|為に[はも]|わけ(では|じゃ(?!あ))|
    いきなり|すれば|れば(?=([い良善好]い|[よ良善好]か))|て(?=い?ました)|しっかり|して(?=あげ([るた]|(ます|まし)))|て(?=(ください|下さい))|これまでに(?!は)|
    より(?=ずっと)|はじめて|て(?=くれ)|くなって(?!は)|され[るた](?![んの])|かった(?![んのりわ])|もなくて(?!は)|あらゆる|すべて(の|を|では|じゃ(?!あ))|
    もなく(?!て)|ながら|がてら|った(?![らんのりわ])|よりも|かも(?=[しれ])|とともに(?![はも])|と共に(?![はも])|もっとも|すべて(?!でのを)|ただの|まま(?=([一-龯]|[゠-ヿ]{2}))|
    どうして|どうやって|した(?=([一-龯]{2}|こと|とこ))|のもとに|[うくすつぬふむるじの]よう[にな]|れて(?=(いき?ま|いる|いた|いな))|じゃ(?=な[いか])|では(?=な[いか])|
    ...
    ゆっくり(?=([一-龯]|[゠-ヿ]{2}))|ちゃんと(?=([一-龯]|[゠-ヿ]{2}))|(なければ|なきゃ)(?=(なら|いけ))|[ぁ-ゖ゛-ゟ](?=(はず|べき)(だ|よ|$|。|…|！|？))|[ぁ-ゖ゛-ゟ](?=[゠-ヿ]{2})|て(?=ありがと)|
    なら(?=([一-龯]|[゠-ヿ]{2}))|なのは|[えけげせぜてでねめれ]る(?=([一-龯]|[゠-ヿ]{2}))|たく(?=な[いか])|[わかさたなまら]れ[るた](?=([一-龯]|[゠-ヿ]{2}))|いくつか|[一-龯]ても|して(?=([一-龯]|[゠-ヿ]{2}))|
    [一-龯]たる(?=([一-龯]|[゠-ヿ]{2}))|という(?=([一-龯]|[゠-ヿ]{2}))|を|な[くい](?=([一-龯]|[゠-ヿ]{2}))|[一-龯][ぁ-ゖ゛-ゟ]に(?=な(る|った|らな))
)
""", re.VERBOSE)
