...
(
    ([一-龯]{1,2}|[゠-ヿ]{2,12}|こと|ところ|[一-龯](?:[ぁ-ゖ゛-ゟー](?!で))+[一-龯]|[゠-ヿ]{2,12}[一-龯]|もの|入り|」|たち|ここ|そこ|[一-龯]ら|(?P<double>[ぁ-ゖ゛-ゟー]{2})(?P=double)|[えけげせぜてでねめれ]る)
    (が|か(?![はもらえけげせぜてでねめれいきぎしちにんを])|か[は]|は(?!ず)|も|の(?![にがはた為])|なく(?!て)|な(?![くのん])|する(?!な)|から(?!して)|まで|に(?![はも])|
    に[はも]|へ|で(?![はもすき])|で[はも]|じて|や|との|と(?![のな])|して[はも]|して(?!はも))
    |
    [、。？！・：；]
    |
    (――)|(……)|(\.\.\.)
    ...
    [一-龯]すぎ[^たるだ]
    |
    について(?![はも])|について[はも]|に関して(?![はも])|に関して[はも]|[っいきぎしちにん][ただ]り|とにかく|でも|[くぐ]らい|まるで|って(?![るたか])|
    すなわち|[うくぐすつぬふむる]の[にはもが]|を|んな[のに]|ったら|として|つまり|ちょっと|ちょうど|々な|々に(?![もは])|々に[もは]|
    だと|だけ|とは|[のただ]ほうが|ないほうが|[のただ]方が|ない方が|風に|[いきしちにひみり]たくて|[うくすつぬふむる]まて|[^一-龯]続く|
    ...
    もなく(?!て)|ながら|がてら|った(?![らんのり])|よりも|かも(?=[しれ])|とともに(?![はも])|と共に(?![はも])|もっとも|すべて(?!でのを)|
    どうして|どうやって|[一-龯]{2}した(?=([一-龯]{2}|こと|とこ))|のもとに|[うくすつぬふむるじの]よう[にな]|れて(?=(いき?ま|いる|いな))|
    どうなるか(?!は)|どうなるかは|しばらく|[えけげせぜてでねめれ]なく(?!て)|[えけげせぜてでねめれあかさたなまら]ずに|[えけげせぜてでねめれいきしじちにみりっ]て(?=い(る|ま|く|け))|
    [一-龯]い(?=([一-龯]|[゠-ヿ]{2}))|[一-龯]しく(?=([一-龯]|[゠-ヿ]{2}))|べきじゃ(?!あ)|かなり(?=([一-龯]|[゠-ヿ]{2}))|[えけげせぜてでねめれ]ば(?=([一-龯]|[゠-ヿ]{2}))|
    ゆっくり(?=([一-龯]|[゠-ヿ]{2}))|ちゃんと(?=([一-龯]|[゠-ヿ]{2}))
)
""", re.VERBOSE)
