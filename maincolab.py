...
(
    ([一-龯]{2}|[゠-ヿ]{2,12}|こと|ところ|[一-龯](?:[ぁ-ゖ゛-ゟー](?!で))+[一-龯]|[゠-ヿ]{2,12}[一-龯])
    (が|か(?![はもら])|か[は]|は|も|の(?![にがはた為])|なく(?!て)|な(?![くのん])|する(?!な)|から(?!して)|まで|に(?![はも])|
    に[はも]|へ|で(?![はもすき])|で[はも]|じて|や|との|と(?![のな])|しては|して(?!は))
    |
    [、。？！・：；]
    |
    (――)|(……)|(\.\.\.)
    ...
    [一-龯]すぎ[^たるだ]
    |
    について(?!は)|については|に関して(?!は)|に関しては|ったり|とにかく|でも|[くぐ]らい|まるで|って(?![るたか])|
    すなわち|[うくすつぬふむる]の[にはもが]|を|んな[のに]|ったら|として|つまり|ちょっと|ちょうど|
    だと|だけ|とは|[のただ]ほうが|ないほうが|[のただ]方が|ない方が|風に|[いきしちにひみり]たくて|
    ほとんど|らしくて(?!は)|らしく(?!て)|ために(?![はも])|ために[はも]|為に(?![はも])|為に[はも]|
    いきなり|すれば|れば(?=[い良善好]い)|て(?=い?ました)|しっかり|して(?=あげ([るた]|(ます|まし)))|
    より(?=ずっと)|はじめて|て(?=くれ)|くなって(?!は)|され[るた](?![んの])|かった(?![んのり])|もなくて(?!は)|
    もなく(?!て)|ながら|がてら|った(?!らんのり)|よりも|かも(?=しれ)|とともに(?!はも)|と共に(?!はも)|もっとも|
    どうして|どうやって
)
""", re.VERBOSE)
